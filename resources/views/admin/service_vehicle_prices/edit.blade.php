...
@section('content')
<h1>Editar Precio Servicio - Vehículo #{{ $price->id }}</h1>
...
<form action="{{ route('service_vehicle_prices.update', $price->id) }}" method="POST" class="space-y-4 max-w-md">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <select name="vehicle_type_id" required class="w-full border rounded px-3 py-2">
            ...
            @foreach ($vehicleTypes as $vehicle)
                <option value="{{ $vehicle->id }}" {{ (old('vehicle_type_id', $price->vehicle_type_id) == $vehicle->id) ? 'selected' : '' }}>
                    {{ $vehicle->name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input type="number" step="0.01" name="price" value="{{ old('price', $price->price) }}" required class="w-full border rounded px-3 py-2" />
    </div>
